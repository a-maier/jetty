...
use jetty::{anti_kt_f, cluster::naive::ClusterNaive, ClusterStep, PseudoJet};
...
fn main() -> Result<()> {
    ...
    let events: Vec<Vec<PseudoJet>> = events
        .into_iter()
        .map(|ev| ev.into_iter().map(|p| p.into()).collect())
        .collect();
    ...
    let njets: usize = events
        .into_iter()
        .map(|ev| {
            let cluster = ClusterNaive::new(ev, anti_kt_f(0.4));
            cluster
                .filter(|s| match s {
